...
```{r}
dtm2 = weightTfIdf(dtm) #normalize the data for word frequency ***** , the higher the number the less frequent it occurred 
```
...
```{r, preparing the data for kmeans}
preproc = preProcess(m) 
m_norm = predict(preproc, m)

set.seed(41) #creating a seed to keep the kmeans the same
cl = kmeans(m_norm, centers = 6)
```

```{r, Creating the kmeans}
set.seed(41) #creating a seed to keep the kmeans the same  
...
# cat("\n\nFatalities in cluster 1:")
# sum(Plane$Fatalities[which(cl$cluster==1)])
# sum(Plane$Survivors[which(cl$cluster==1)])
# print('Fatalities in cluster 2:')
# sum(Plane$Fatalities[which(cl$cluster==2)])
# sum(Plane$Survivors[which(cl$cluster==2)])
# print('Fatalities in cluster 3:')
# sum(Plane$Fatalities[which(cl$cluster==3)])
# sum(Plane$Survivors[which(cl$cluster==3)])
# print('Fatalities in cluster 4:')
# sum(Plane$Fatalities[which(cl$cluster==4)])
# sum(Plane$Survivors[which(cl$cluster==4)])
# print('Fatalities in cluster 5:')
# sum(Plane$Fatalities[which(cl$cluster==5)])
# sum(Plane$Survivors[which(cl$cluster==5)])
# print('Fatalities in cluster 6:')
# sum(Plane$Fatalities[which(cl$cluster==6)])
# sum(Plane$Survivors[which(cl$cluster==6)])
# print('Fatalities in cluster 7:')
# sum(Plane$Fatalities[which(cl$cluster==7)])
# sum(Plane$Survivors[which(cl$cluster==7)])
# print('Fatalities in cluster 8:')
# sum(Plane$Fatalities[which(cl$cluster==8)])
# sum(Plane$Survivors[which(cl$cluster==8)])
# print('Fatalities in cluster 9:')
# sum(Plane$Fatalities[which(cl$cluster==9)])
# sum(Plane$Survivors[which(cl$cluster==9)])
# print('Fatalities in cluster 10:')
# sum(Plane$Fatalities[which(cl$cluster==10)])
# sum(Plane$Survivors[which(cl$cluster==10)])
```
...
labs(caption = "Date FAA was established: 08-23-1958") + #adding caption to graph
...
ylab("Percentage") + #changing y-label
scale_fill_manual(values = c("Red","Green")) + #Changing colors on the bars 
labs(caption = "Date FAA was established: 08-23-1958") +
geom_text(aes(label=signif(value, 5)), position=position_dodge(width=0.9), vjust=-0.25) + #adding statistic above bar graph to show distinguishability
scale_y_continuous(limits = c(0.0,1.0) , breaks = seq(0,1.0,.2)) 
```
